...
class Main {
    ...
    //[card: [count,idx]
    static HashMap<ArrayList<Integer>, ArrayList<Integer>> map = new HashMap<>();
            ...
            for (int friend = 0; friend < friends; friend++) {
                ...
                for (int i = 0; i < friendCards.length; i += 2) {
                    ...
                    if(!cardCounts.containsKey(inCard)) {
                    	...
                    	outCard.add(value);
                      	outCard.add(color);
                      	ArrayList<Integer> mapVal = new ArrayList<>();
                      	mapVal.add(1);
                      	mapVal.add(numNodes++);
                      	map.put(inCard, mapVal);
...
//                    cardCounts.put(inCard, cardCounts.getOrDefault(inCard, 0)+1);
        ...
        int edgeIndex = 0;
        for(int node = 0; node < cards.size(); node++) {
        	if(node%2==0) {
        		 // Add edge from s to in_{i,x,c}  if x == 1 with capacity infinity
        		if(cards.get(node).get(1)==1) {
            		dests_rough.get(srcIndex).add(node);
            		dests_rough.get(node).add(srcIndex);
            		ids_rough.get(srcIndex).add(edgeIndex++);
            		ids_rough.get(node).add(edgeIndex++);
        		...
        		int c = cardCounts.get(cards.get(node));
            	dests_rough.get(node).add(node+1);
        		dests_rough.get(node+1).add(node);
        		ids_rough.get(node).add(edgeIndex++);
        		ids_rough.get(node+1).add(edgeIndex++);
        		capacities_rough.add(c);
        		capacities_rough.add(0);
        	} else {
        		// Add edge from out_{i,x,c} to t if x == m with capacity infinity
        		if(cards.get(node).get(1)==values) {
            		dests_rough.get(node).add(sinkIndex);
            		dests_rough.get(sinkIndex).add(node);
            		ids_rough.get(node).add(edgeIndex++);
